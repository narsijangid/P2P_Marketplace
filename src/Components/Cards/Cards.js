...
const Cards = ({ product }) => {
  ...
  const deleteAd = () => {
    let name = imgURL.substr(
      imgURL.indexOf('%2F') + 3,
      imgURL.indexOf('?') - (imgURL.indexOf('%2F') + 3)
    );
    ...
    firebasestorage
      ...
      .child(`image/${name}`)
      .delete()
      .then(() => db.collection('products').doc(`${product.id}`).delete())
      .catch(() => console.log('Error deleting ad'));
  };
  ...
  return (
    <div key={product.id} className="card__container">
      {showPopup && <div className="popup">Added to Favorites ✅</div>}
      ...
      <div className="card__deleteBtn">
        {location.pathname === '/myads' && (
          <>
            <i
              onClick={() => history.push(`/editpost/${product.id}`)}
